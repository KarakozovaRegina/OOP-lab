...
namespace lab9
{
    enum AgeGroup//используеться как поле для класса
    {
    ...
    //1 задание(класс +вложенная в него коллекция)класс-коллекция
    class Collection<T> : IEnumerable<T>
    {
        ...
        //собираем обЪекты класса в коллекцию?
        ...
        public void DeleteByName(string _Name)     //метод удаления в массив элемента
        {
            int position = 0;
            bool flag = false;
        ...
        public void DeleteByIndex(int _index)          //метод удаления в массив элемента
        {
            if (_index != count)
            {
